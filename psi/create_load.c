...
/* These defines are the number of seconds for which we load CPU or I/O */
#define CPU_LOAD_TIME_SECS  10
#define IO_LOAD_TIME_SECS   10

/* We split the list of directories to traverse between 2 I/O
   loader threads. This scruct is passed to each of them,
...
/* One function that prints the system call and the error details, and then
   exits with error code 1. Non-zero meaning things didn't go well. */
void
fatal_error(const char *syscall)
{
    perror(syscall);
    exit(1);
}
...
**get_root_dir_entries()
{
    ...
    /* We must NULL-terminate the list */
    entries = realloc(entries, sizeof(char *) * (i + 1));
    entries[i] = NULL;
...
void
read_dir_contents(char *dir_path)
{
    struct dirent *entry;
    struct stat st;
    char buff[16384];
...
/* This function is called in a thread. It iterates through the list
...
load_disk()
{
    ...
    char **roor_dir_entries = get_root_dir_entries();
    while (roor_dir_entries[i++] != NULL);

    dir_list dl1, dl2;
    dl1.dirs = roor_dir_entries;
    dl1.begin_idx = 0;
    dl1.count = i/2;

    dl2.dirs = roor_dir_entries;
